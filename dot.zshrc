...
if [[ ! -d $ZI_HOME ]]; then
    print -P "%F{33}▓▒░ %F{220}Installing %F{33}DHARMA%F{220} Initiative Plugin Manager (%F{33}weijing24/zi%F{220})…%f"
    command mkdir -p "$ZI_HOME" && command chmod g-rwX "$ZI_HOME"
    command git clone https://github.com/weijing24/zi "$ZI_HOME/bin" && \
        print -P "%F{33}▓▒░ %F{34}Installation successful.%f%b" || \
        print -P "%F{160}▓▒░ The clone has failed.%f%b"
fi
...
source /Users/tru/Dropbox/git/src/github.com/tru2dagame/zsh-history-substring-search/zsh-history-substring-search.plugin.zsh

zi wait lucid for \
    atinit"ZI[COMPINIT_OPTS]=-C; zicompinit; zicdreplay" z-shell/F-Sy-H \
    bindmap"^R -> ^H" z-shell/H-S-MW \
    blockf zsh-users/zsh-completions \
    blockf wbingli/zsh-claudecode-completion \
...
zi snippet /Users/tru/Dropbox/git/src/github.com/tru2dagame/zsh-histdb-fzf/fzf-histdb.zsh
